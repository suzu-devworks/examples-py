...
def do_sample(args: any):
    ...
    def to_fraction(value) -> str:
        if value < 0.3:
            return str(Fraction(value))

        return str(value)

    def to_coordinate(ref, value):
        degree = int(value[0])
        minute = int(value[1])
        second = value[2]
        return f"{degree}°{minute}'{second}\"{ref}"

    def loginfo_all(list):
    ...
    with Image.open(path) as image:
        ...
        expouse = to_fraction(exif_private[EXIFTAGS.ExposureTime])
        logger.info(f"----- ShutterSpeedValue    : {shutter}")
        logger.info(f"----- ExposureTime         : {expouse}")
        logger.info("\n")
