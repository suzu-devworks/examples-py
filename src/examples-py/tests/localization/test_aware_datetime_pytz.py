...
- https://qiita.com/higitune/items/0ca244373d380cf1c060
"""
...
def test_datetime_generators():
    ...
    assert utc.tzinfo.tzname(nativedate) == pytz.UTC.tzname(nativedate)
    ...
    assert local.tzinfo.tzname(nativedate) == tz_ja.tzname(nativedate)
...
def test_datetime_convert_timezone_dst():
    ...
    local_eafter = est.localize(edt_after)
    normal_after = est.normalize(est.localize(edt_after))
    assert local_eafter.tzinfo == normal_after.tzinfo
...
def test_datetime_convert_native():
    ...
    assert aware.tzinfo.zone == tz_ja.zone
    ...
    aszone = native.astimezone(tz=tz_ja)
    assert aszone.tzinfo is not None
    assert aszone.tzinfo.zone == tz_ja.zone
    assert str(aszone) == "2000-02-29 10:23:45.678901+09:00"
    assert aszone != aware
